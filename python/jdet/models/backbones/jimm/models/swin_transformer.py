...
from .layers import PatchEmbed, Mlp, DropPath, to_2tuple, trunc_normal_, AdaptiveAvgPool1d
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.H_pad = input_resolution[0] if input_resolution[0]%self.window_size==0 else (input_resolution[0]//self.window_size+1)*self.window_size
        self.W_pad = input_resolution[1] if input_resolution[0]%self.window_size==0 else (input_resolution[1]//self.window_size+1)*self.window_size
        
        if self.shift_size > 0:
            # calculate attention mask for SW-MSA
            
            img_mask = jt.zeros((1, self.H_pad, self.W_pad, 1))  # 1 H W 1
    ...
    def execute(self, x):
        ...
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        x = nn.pad(x, (0, 0, 0, self.pad_r, 0, self.pad_b))
        # cyclic shift
        if self.shift_size > 0:
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=1024, patch_size=4, in_chans=3, num_classes=1000,
        ...
        if self.ape:
            self.absolute_pos_embed = jt.zeros((1, num_patches, embed_dim))
