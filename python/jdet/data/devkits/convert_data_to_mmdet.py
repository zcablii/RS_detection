...
from jdet.config.constant import get_classes_by_name, FAIR1M_MAPPING
...
def parse_ann_info_map(label_base_path, img_name, label_ids, angle_version):
    lab_path = osp.join(label_base_path, img_name + '.txt')
    bboxes, labels, bboxes_ignore, labels_ignore = [], [], [], []
    with open(lab_path, 'r') as f:
        for ann_line in f.readlines():
            ...
            # 8 point to 5 point xywha
            bbox = tuple(poly_to_rotated_box_single(bbox, angle_version).tolist())
            class_name = FAIR1M_MAPPING[ann_line[8]]
            difficult = int(ann_line[9])
            # ignore difficult =2
            if difficult == 0:
                bboxes.append(bbox)
...
def convert_data_to_mmdet(src_path, out_path, trainval=True,
                          filter_empty_gt=True, ext='.png', type='',
                          angle_version='le135'):
...
def convert_data_to_mmdet_mapping(src_path, out_path, trainval=True,
                                  filter_empty_gt=True, ext='.png', type='',
                                  angle_version='le135'):
    """Generate .pkl format annotation that is consistent with mmdet.
    Args:
    ...
    for img in tqdm(img_lists):
        ...
        if trainval:
            ...
            if filter_empty_gt & (osp.getsize(label) == 0):
                continue
            bboxes, labels, bboxes_ignore, labels_ignore = parse_ann_info_map(
                label_path, img_name, label_ids, angle_version)
            ann = {}
            ann['bboxes'] = np.array(bboxes, dtype=np.float32)
            ann['labels'] = np.array(labels, dtype=np.int64)
            ann['bboxes_ignore'] = np.array(bboxes_ignore, dtype=np.float32)
